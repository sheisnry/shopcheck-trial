export default async function handler(req, res) {
  ...
  const developerInstruction = [
    ...
    // DB SCHEMA AWARENESS — every full_report field becomes a direct DB column in shopcheck_full_report_readable_v1 and a Canva slot. Treat length/count limits in the user prompt as hard constraints.
    'DB-AWARE OUTPUT: The JSON you return for step 2 is directly exported as flat columns (overview, focus_1/2/3_title/why/expected_result/steps, week_1/2/3/4_label/title/details, channel_fit_score_pct/label/long_reason/conclusion_lines) and each column is a slot in a 10-slide Canva report. Respect the per-field length and array-count limits in the prompt EXACTLY — wrong count (e.g. 2 or 4 items where 3 is required) breaks the report. Order focus_areas by impact strictly high → low. Ensure plan_30_days[].details is exactly 3 items per week. Ensure channel_fit.conclusion_lines is exactly 3 items.',
    isStep2
